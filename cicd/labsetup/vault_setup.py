...
def create_kv1_secrets(client):
    ...
    kv1_s = {
        'private_key': readfile(f"{VAULT_DIR}/key1k.pem"),
...
def create_kv2_secrets(client):
    ...
    for app in ["SampleTenant/App1", "SampleTenant/App2", "AnotherTenant/AnotherApp1"]:
        # create secret with 1k key
        kv2_s = {
            'private_key': readfile(f"{VAULT_DIR}/key1k.pem"),
            'certificate': readfile(f"{VAULT_DIR}/cert1k.pem"),
        }
        client.secrets.kv.v2.create_or_update_secret(mount_point='secretv2', path=f'{app}/x509', secret=kv2_s)
        # update secret with 2k key
        kv2_s = {
            'private_key': readfile(f"{VAULT_DIR}/key2k.pem"),
            'certificate': readfile(f"{VAULT_DIR}/cert2k.pem"),
        }
        client.secrets.kv.v2.create_or_update_secret(mount_point='secretv2', path=f'{app}/x509', secret=kv2_s)
        # create aes key
        client.secrets.kv.v2.create_or_update_secret(
            mount_point='secretv2', path=f'{app}/sharedsecret',
...
def log_kv2_secrets(client, vault_log):
    ...
    secrets = (
        {'mount_point':'secretv2','path':'SampleTenant/App1/x509','version':1},
        {'mount_point':'secretv2','path':'SampleTenant/App1/x509','version':2},
        {'mount_point':'secretv2','path':'SampleTenant/App2/x509','version':1},
        {'mount_point':'secretv2','path':'SampleTenant/App2/x509','version':2},
        {'mount_point':'secretv2','path':'AnotherTenant/AnotherApp1/x509','version':1},
        {'mount_point':'secretv2','path':'AnotherTenant/AnotherApp1/x509','version':2},
        {'mount_point':'secretv2','path':'SampleTenant/App1/sharedsecret','version':1},
        {'mount_point':'secretv2','path':'SampleTenant/App1/sharedsecret','version':2},
        {'mount_point':'secretv2','path':'SampleTenant/App2/sharedsecret','version':1},
        {'mount_point':'secretv2','path':'SampleTenant/App2/sharedsecret','version':2},
        {'mount_point':'secretv2','path':'AnotherTenant/AnotherApp1/sharedsecret','version':1},
        {'mount_point':'secretv2','path':'AnotherTenant/AnotherApp1/sharedsecret','version':2},
    )
